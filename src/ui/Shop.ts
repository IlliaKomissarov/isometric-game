...
export class ShopUI {
  ...
  private render(): void {
    ...
    this.panel.innerHTML = `
      ...
      <div class="tp-cols">
        <div class="tp-col"><h4>FOR SALE</h4><div class="tp-list">${sale || '<span class="tp-empty">Sold out</span>'}</div></div>
        <div class="tp-col"><h4>YOUR PACK · sell</h4><div class="tp-list">${pack || '<span class="tp-empty">Nothing to sell</span>'}</div></div>
      </div>
      <div class="tp-note">Click an item to buy or sell · ESC closes</div>`;
    ...
    wireItemTips(this.panel, ITEMS, (def) => {
      const inPack = p.backpack.includes(def.id) && !this.town.stock.includes(def.id);
      return inPack ? `Sells for ${this.town.sellPrice(def)} gold` : `Buy for ${this.town.buyPrice(def)} gold · sells for ${this.town.sellPrice(def)}`;
    });
  }
