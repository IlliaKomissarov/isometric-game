...
- **Project version:** 0.2.0 (iteration 117, 2026-09-18)
...
| `engine/Lighting` | dungeon grid, player tile | per-tile tint, visible set, cutaway | owns sprite `visible` for fog AND sprite `tint` for everything registered - a dresser that wants a prop permanently darker or brighter than its tile says so through `registerProp`'s per-channel `shade` (it.111); writing `sprite.tint` after registering is dead code. A floor may hand in its own `warmRgb`/`shadowRgb` and change the character of every sprite on it at once (it.111). `omniscient` is the COLISEUM's switch only (it.53); it.107 defaulted it on game-wide and it.109 put it back. The farmlands and the riverside call `revealAll` deliberately - open country is drawn from the first frame (it.103) - which is a different thing from having no fog |
...
| the siege engines (it.110, given the real machine it.112, `town/TownProps`) | `scripts/bake-catapult.py`'s eight stacked facings + its arm, sling and stone; `TownDressing.fireSiege` | a catapult that faces its aim, an arm that swings, a BURNING stone in the air shedding embers, and an `onImpact` main damages through | render-only: the dressing throws the stone, main deals the blow through `dealDamage`, so the sole-hp-mutator rule holds. The body's anchor and scale come from the bake's own printed ground centre - never guessed |
| `render/RiverWater` (it.106, cross-fade it.110b, current it.111) | the baked caustic frames, the ground sprites the scene made | a river that RUNS: every tile holds a second sprite with the NEXT frame and the pass smoothsteps between them, and every phase is baked with its field shifted a tenth of a period downstream, so ten phases carry the pattern exactly three tiles and wrap | render-only, wall clock; the dissolve layer is told the tint the LIGHTING wrote and the `renderable` the CULLER wrote, because neither of them knows it exists |
...
| Finding | Severity | Fix |
| --- | --- | --- |
| The bridge STILL read as a ladder lying on the water | High (visual) | it.110b used the right pack and the wrong idea: a roadway resting ON the surface with nothing under it is not a bridge whatever it is paved with. `scripts/bake-bridge.py` composes bay (roadway + underside + BOTH parapets, lapped), arched pier, abutment, post and gate against the exact projection the renderer uses - pack to game is `x*0.25`, `y*0.25 - (h*0.25 - 32)`, so nothing is fitted by hand |
| The piers were invisible when they were one tile wide | High (visual) | the deck's own underside and the next bay's leading edge cover every pixel of a pier the width of its road, which is why the first it.111 span looked like a wall with notches. The pier is baked a third wider (`PIER_FLARE`), which is also what a pier is |
| Both "gates" were half an arch | Medium (visual) | `arch/big` ships as two halves on one canvas. it.110b baked one. They are composited a tile apart along the `s` run - the one that crosses a road running +x - so the hero walks UNDER the opening |
| A shelf of lit grass and beach lay along the whole far bank | High (visual) | it.110b stopped PLANTING across the water but left the meadow's lobes reaching over the course. Everything across it is unmade now (`TILE_WALL`, which the scene never draws); the water is widened four tiles over there and takes a black `deepfade_*` at every edge that touches the void. 0 far-bank tiles at five seeds, and the landing is the only exception |
| The waterline was a pale sawtooth ruled the length of the map | Medium (visual) | the sand band's wobble is a pair of sines, and a SMOOTH boundary between two tile kinds draws in this projection as a clean zigzag. A per-tile jitter of half a tile makes it a speckled margin |
| Two full-canopy trees stood in front of the bridge gate | Medium (visual) | buildings were kept off the crossing in it.110; the border wood was not. Ten tiles of it are clear now - and the apron is carved as LAND first, because the belt seeds on everything that is not land, so refusing to plant left a bald VOID wedge where the trees had been |
| The river was a mesh of bright cyan cracks | High (visual) | the caustic loop at full strength on a near-black body. The body is deeper and greener with a swell sampled over thirteen tiles in world space, and the glint is a quarter of what it was through a 3.4 gamma - only the crests of the pattern show |
| The water shimmered in place instead of running | Medium (visual) | every phase is baked with its whole field shifted a tenth of a source period downstream; ten phases carry it exactly three tiles and wrap, so the loop still closes. `RiverWater` period 2.6 → 5.5 s, which puts the current at about half a tile a second |
| The battlefield's blood was pink smoke | High (visual) | a soft alpha on red-minus-the-rest keeps every pale plaster pixel of the source photograph. `scripts/bake-blood.py` thresholds hard on red excess and then THROWS THE COLOUR AWAY, re-shading each pool from its own thickness. Six pools, three fine spatters from the tileset's own ground splats, ~380 marks on the field |
| The field was lit like a taproom | High (visual) | every floor's light ramp ended at the same warm candle white. `Lighting.build` takes `warmRgb`/`shadowRgb`; the field's is a cold drained grey over near-black. Explored 0.14 → 0.10, torch pool 10 → 9, pasture down from a third of the map to a tenth |
| The dead were bright blue lozenges, and had been since it.91 | High (bug) | Lighting OWNS a registered prop's tint and rewrites it from the tile every frame, so every `body.tint` written in the corpse dresser was dead code. `registerProp` takes a PER-CHANNEL `shade` multiplied into the light; the city's dead are pulled hardest on the blue, so they lose the colour as well as the light |
| The basement door was a leaf in a thirty-two-tile wall and nothing else | Medium (design) | a flagged threshold, the only pair of sconces on the run, and - while it is barred - the bench the merchant says they kept across it |
| The merchant wore a sheet the player sees in the market twice a minute | Medium (design) | all seven `cit_*` sheets are `STREET_FOLK`. `scripts/bake-trader.py` re-dyes the one citizen carrying a bundle into `trader_walk`, a wine-and-plum livery worn nowhere else; he is a head taller than the townsfolk, lifted OUT of the room's light, with his recovered goods at his feet |
| Brack was still negotiating | Low (writing) | he offered the door and took it back, and a man who explains why he is dangerous is not one. He has decided before the hero is through it now, and prices the corpse aloud |
| One 404 in the manifest silently disables the whole atlas | Medium (robustness) | the loader fetches every single at boot and the failure surfaces as one console warning while the game falls back to procedural art. A bake that supersedes a kit deletes the old KEYS as well as the old files |
| A `shade` over 1 threw inside the render pass | Medium (bug) | `shadeTint` clamps each channel; an unclamped one makes a number Pixi refuses to read as a colour |
